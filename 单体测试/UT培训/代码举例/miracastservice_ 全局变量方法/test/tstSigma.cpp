...
class tstWfdReplier : public IWFDServiceProxyReplier
{
    public:
    tstWfdReplier()
 //       :m_Proxy(new WFDServiceProxyBase(this))
    {
...
private:
//    goni::WFDServiceProxyBase* m_Proxy;
...
class wfdSigmaService : public ServiceBase
{
public:
...
//        mSub = new goni::WFDServiceSub(new wfdTestSub(), this);
    ...
    public:
        ...
        static wfdSigmaService* m_SigmaService;
        static wfdSigmaService* instance();
...
wfdSigmaService* wfdSigmaService::m_SigmaService = NULL;

wfdSigmaService::wfdSigmaService()
:ServiceBase("wfdSigma")
{
    MCLOGD("**********%s*******\n", __func__);
...
void wfdSigmaService::onPlay()
{
    mProxy->wfd_play();
}

void wfdSigmaService::onPause()
{
    mProxy->wfd_pause();
}

void wfdSigmaService::onDisconnect()
{
    mProxy->wfd_disconnect();
}

wfdSigmaService* wfdSigmaService::instance()
{
    if(m_SigmaService == NULL) {
       m_SigmaService = new wfdSigmaService();
    }
    return m_SigmaService;
}



void wfdSigmaService::greetings()
{
    printf("----------------------------\r\n");
    printf(" 1. Power On\r\n");
    printf(" 2. Power off\r\n");
...
void wfdSigmaService::MyGetLine(char *p, int len)
{
    if (NULL == p) {
        return;
    }
...
int wfdSigmaService::checkInput(char *p)
{
    if (p == NULL) {
        return 0;
    }
...
static tstWfdReplier* tstReplier;
//Process process("testWfdService_Sigma");
wfdSigmaService* sigmaService = NULL;
static WFDServiceProxyBase* mWFDProxy = NULL;
int init()
{
    printf("enter initService \r\n");
    //tstReplier = new tstWfdReplier();
    //sigmaService = new wfdSigmaService();
    //process.addService(sigmaService);
    //process.initialize();
    tstReplier = new tstWfdReplier();
    printf("enter initService1 \r\n");
    mWFDProxy = new WFDServiceProxyBase(tstReplier,1);
    printf("enter initService2 \r\n");
    if (mWFDProxy != NULL) {

        while (!mWFDProxy->tryConnect()) {
            sleep(1);
        }
    }
    return 0;
...
{
    MCLOGD("**********%s*******\n", __func__);
    //sigmaService->onDisconnect();
    mWFDProxy->wfd_disconnect();
    return 0;
}
...
{
    MCLOGD("**********%s*******\n", __func__);
    //if (sigmaService!= NULL) delete sigmaService;
    if (mWFDProxy!= NULL) delete mWFDProxy;
    return 0;
}
